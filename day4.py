...
def find(text):
    ...
    def hasValidKeys(d):
        vkeys = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];
        for vk in vkeys:
            if vk not in d:
                return False;
            if vk in ["byr", "iyr", "eyr"]:
                if len(d[vk]) != 4:
                    return False
                elif (vk == "byr" and not (1920 <= int(d[vk]) <= 2002)) or \
                        (vk == "iyr" and not (2010 <= int(d[vk]) <= 2020)) or \
                        (vk == "eyr" and not (2020 <= int(d[vk]) <= 2030)):
                    return False
            elif vk == "hgt":
                t = d[vk].strip();
                if not (len(t) > 2 and (t[-2:] == "cm" and 150 <= int(t[:-2]) <= 193 or \
                                        t[-2:] == "in" and 59 <= int(t[:-2]) <= 76)):
                    return False
            elif vk == "hcl":
                t = d[vk].strip();
                if not (t[0] == '#' and len(t) == 7):
                    return False
                else:
                    pattern = re.compile('^[0123456789abcdef]+$')
                    if not (check(t[1], pattern)):
                        return False
            elif vk == "ecl" and d[vk] not in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
                return False;
            elif vk == "pid" and not (len(d[vk].strip()) == 9):
                    return False
        return True;

    def isValid(s):
        d = createDict(s);
        if len(d.keys()) < 8 and 'cid' in d:
            return False
        return hasValidKeys(d)

    a = text.split("\n\n");
    c = 0;
    print('Total passports:', len(a));
    for l in a:
        l = l.replace('\n', ' ');
        if isValid(l):
            c += 1
